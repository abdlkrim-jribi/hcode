...
class TestToolManagerIntegration:
    ...
    async def test_read_existing_file(self, tool_manager, temp_project):
        ...
        result = await tool_manager.read_file(
    ...
    async def test_read_nonexistent_file(self, tool_manager, temp_project):
        ...
        result = await tool_manager.read_file(
            str(Path(temp_project) / "nonexistent.py")
        )
...
class TestSearchWorkflow:
    ...
    async def test_glob_and_grep_workflow(self, temp_project):
        ...
        # Find all Python files
        glob_tool = GlobTool(root_dir=temp_project)
        python_files = await glob_tool.execute(
            pattern="**/*.py",
            path=temp_project
        )
...
class TestTodoIntegration:
    ...
    async def test_todo_workflow(self):
        ...
        result1 = await tool.execute(todos=[
            {"content": "Read configuration", "status": "in_progress", "activeForm": "Reading configuration"},
            {"content": "Update settings", "status": "pending", "activeForm": "Updating settings"},
        ...
        result2 = await tool.execute(todos=[
            {"content": "Read configuration", "status": "completed", "activeForm": "Reading configuration"},
            {"content": "Update settings", "status": "in_progress", "activeForm": "Updating settings"},
            {"content": "Test changes", "status": "pending", "activeForm": "Testing changes"}
        ])
        ...
        result3 = await tool.execute(todos=[
            {"content": "Read configuration", "status": "completed", "activeForm": "Reading configuration"},
            {"content": "Update settings", "status": "completed", "activeForm": "Updating settings"},
            {"content": "Test changes", "status": "in_progress", "activeForm": "Testing changes"}
        ])
...
class TestSafetyIntegration:
    ...
    def test_safety_with_commands(self):
        ...
        # Safe commands
        assert safety.assess_command_risk("ls -la") == safety.assess_command_risk("pwd")  # Both SAFE
...
"""
        ...
        fix_result = await edit_tool.execute(
            file_path=bug_file,
            old_string="print(itme)",
            new_string="print(item)"
        )
...
class TestErrorHandling:
    ...
    async def test_graceful_error_handling(self, temp_project):
        ...
        read_result = await read_tool.execute(
            file_path=str(Path(temp_project) / "nonexistent.py")
        )
        ...
        grep_result = await grep_tool.execute(
            pattern="test",
            path=str(Path(temp_project) / "nonexistent_dir")
        )
    ...
    async def test_todo_validation_errors(self):
        ...
        result1 = await tool.execute(todos=[
            {"content": "Task 1", "status": "in_progress"}  # Missing activeForm
        ])
        assert result1.success == False

        # Invalid status
        result2 = await tool.execute(todos=[
            {"content": "Task 1", "status": "invalid", "activeForm": "T1"}
        ])
...
class TestConcurrentOperations:
    ...
    async def test_parallel_reads(self, temp_project):
        ...
        results = await asyncio.gather(*[
            read_tool.execute(file_path=f) for f in files
        ])
