...
class ToolDisplay:
    ...
    def _display_bash(self, arguments: Dict[str, Any], result: Any):
        ...
        self.console.print(
            f"  {self.style.ICON_BASH} [bold]Bash[/bold] " f"[{self.style.TOOL_NAME}]{command}[/]"
        )
    ...
    def _show_bash_output(self, output: str, max_lines: int = 30):
        ...
        for line in lines[:max_lines]:
            if len(line) > 58:
                line = line[:55] + "..."
            self.console.print(
                f"    {self.style.BOX_V} [{self.style.DIM}]{line:<58}[/] {self.style.BOX_V}"
            )
...
class TestToolDisplayModule:
    ...
    def test_read_tool_display(self):
        ...
        result = MockToolResult(success=True, output="line 1\nline 2\nline 3\nline 4\nline 5")
    ...
    def test_bash_tool_display_shows_output(self):
        ...
        result = MockToolResult(success=True, output="file1.py\nfile2.py\nREADME.md")
    ...
    def test_glob_tool_display(self):
        ...
        result = MockToolResult(success=True, output="src/main.py\nsrc/utils.py\nsrc/config.py")
...
class TestStatusLineDisplay:
    ...
    def test_status_line_render(self):
        ...
        display = StatusLineDisplay()
        line = display.render(model="gpt-4", tokens=1000, cost=0.05, status="ready", cwd="/project")
