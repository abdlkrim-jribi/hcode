/**
 * App.tsx — Main application shell for Hcode Desktop.
 * Resizable 3-column IDE layout: Explorer (left) → Editor (center) → Agent (right).
 * Full state machine driving the unified AI task stream.
 */
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'ACCEPT_PATCH': return {
      ...state,
      patches: state.patches.filter(p => p.path !== action.path),
      appliedPatches: [...state.appliedPatches, action.path],
    };
    case 'REJECT_PATCH': return {
      ...state,
      patches: state.patches.filter(p => p.path !== action.path),
    };
    case 'SET_STREAMING': return { ...state, streamingContent: state.streamingContent + action.content };
    case 'CLEAR_STREAMING': return { ...state, streamingContent: '' };
    case 'SET_CIRCUIT_BREAK': return {
      ...state,
      circuitBreakActive: action.active,
      lastError: action.reason || state.lastError,
      phase: action.active ? 'error' : state.phase,
    };
...
/** Phase 2: Map AgentEvent to state transitions (additive helper) */
function handleAgentEvent(state: AppState, event: AgentEvent): AppState {
  switch (event.type) {
    case 'agent_ready':
      return { ...state, daemonStatus: 'running' };
    case 'task_submitted':
      return { ...state, currentTask: event.task, phase: 'thinking', streamingContent: '' };
    case 'planning_started':
      return { ...state, phase: 'planning' };
    case 'plan_created':
      return { ...state, planMarkdown: event.markdown };
    case 'plan_approved':
      return { ...state, phase: 'executing' };
    case 'execution_started':
      return { ...state, phase: 'executing' };
    case 'task_update':
      return { ...state, streamingContent: event.markdown };
    ...
    case 'file_patch_accepted':
      return {
        ...state,
        patches: state.patches.filter(p => p.path !== event.path),
        appliedPatches: [...state.appliedPatches, event.path],
      };
    case 'file_patch_rejected':
      return {
        ...state,
        patches: state.patches.filter(p => p.path !== event.path),
      };
    ...
    case 'error':
      return { ...state, lastError: event.message, phase: 'error' };
    case 'circuit_break':
      return { ...state, circuitBreakActive: true, lastError: event.reason, phase: 'error' };
    case 'loop_detected':
      return { ...state, lastError: event.message };
    case 'daemon_health':
      return {
        ...state,
        daemonStatus: event.status === 'healthy' ? 'running'
          : event.status === 'degraded' ? 'running'
          : 'error',
      };
    case 'done':
...
export default function App() {
  ...
  // Theme & Editor settings (persisted)
  const [theme, setTheme] = useState<'dark' | 'light'>(() => {
    return (localStorage.getItem('hcode-theme') as 'dark' | 'light') || 'dark';
  });
  const [editorSettings, setEditorSettings] = useState(() => {
    try {
      const saved = localStorage.getItem('hcode-editor-settings');
      return saved ? JSON.parse(saved) : { fontSize: 13, wordWrap: false, minimap: true, lineNumbers: true, tabSize: 4 };
  ...
  useEffect(() => {
    ...
    localStorage.setItem('hcode-theme', theme);
  }, [theme]);

  // Persist editor settings
  useEffect(() => {
    localStorage.setItem('hcode-editor-settings', JSON.stringify(editorSettings));
  }, [editorSettings]);

  // Recent folders helper
  const recentFolders = useMemo(() => {
    try { return JSON.parse(localStorage.getItem('hcode-recent-folders') || '[]'); }
    catch { return []; }
  }, [state.workDir]);
  ...
  useEffect(() => {
    ...
    ipc.onDaemonMessage((msg: HcodeMessage) => {
      switch (msg.type) {
        ...
        case 'circuit_break':
          ...
          dispatch({ type: 'ADD_CHAT', msg: { id: crypto.randomUUID(), type: 'system', content: `Circuit break: ${msg.payload.reason}`, timestamp: new Date().toLocaleTimeString() } });
          break;
        case 'task_update':
    ...
    // Attempt to start daemon on mount
  ...
  // ── Keyboard Shortcuts ────────────────────────────────────────────────────
  // Use a ref for callbacks that are defined later to avoid forward-reference issues
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Ctrl+K: Focus Agent input (new task)
      ...
      // Escape: close overlays or return to editor
      if (e.key === 'Escape') {
  ...
  const handleSubmitTask = useCallback(async (task: string, mode: 'planning' | 'fast') => {
    // Reset ALL previous task state so the UI starts fresh
    ...
    try {
      ...
      dispatch({ type: 'SET_LAST_ERROR', message: err instanceof Error ? err.message : 'Failed to submit task' });
    }
    setFocus('agent');
  }, []);
  ...
  const handleApprovePlan = useCallback(async () => {
    ...
    await ipc.approvePlan();
  }, []);

  const handleRejectPlan = useCallback(async () => {
    dispatch({ type: 'SET_PHASE', phase: 'idle' });
    await ipc.rejectPlan('User rejected plan');
  }, []);

  const handleFileDecision = useCallback(async (path: string, accepted: boolean) => {
    try {
      if (accepted) {
        await ipc.acceptPatch(path);
        dispatch({ type: 'ACCEPT_PATCH', path });
      } else {
        await ipc.rejectPatch(path);
        dispatch({ type: 'REJECT_PATCH', path });
      }
    } catch (err) {
      dispatch({ type: 'SET_LAST_ERROR', message: `Failed to ${accepted ? 'accept' : 'reject'} patch: ${err instanceof Error ? err.message : 'Unknown error'}` });
    }
  }, []);

  const handleRollback = useCallback(async () => {
    try {
      await ipc.rollbackAll();
      dispatch({ type: 'CLEAR_PATCHES' });
      setShowDiffReview(false);
    } catch (err) {
      dispatch({ type: 'SET_LAST_ERROR', message: `Rollback failed: ${err instanceof Error ? err.message : 'Unknown error'}` });
    }
  }, []);

  const handleReviewDiffs = useCallback(() => {
  ...
  const commands: Command[] = useMemo(() => [
    ...
    { id: 'toggle-explorer', label: 'Toggle Explorer', category: 'View', shortcut: 'Ctrl+B', action: () => setExplorerCollapsed(prev => !prev) },
    { id: 'toggle-agent', label: 'Toggle Agent Panel', category: 'View', shortcut: 'Ctrl+J', action: () => setAgentCollapsed(prev => !prev) },
    ...
    { id: 'toggle-theme', label: 'Toggle Theme (Dark/Light)', category: 'Preferences', action: () => setTheme(t => t === 'dark' ? 'light' : 'dark') },
    ...
    { id: 'abort-task', label: 'Abort Current Task', category: 'Agent', action: async () => {
      await ipc.abortTask();
  ...
  const onDragStart = (target: 'explorer' | 'agent', e: React.PointerEvent) => {
    ...
    dragState.current = {
      target,
      startX: e.clientX,
      startWidth: target === 'explorer' ? explorerWidth : agentWidth
    };

    // Add document listeners for mousemove/mouseup (more reliable than just pointer capture)
  ...
  const onDragMove = (e: MouseEvent) => {
    ...
    if (state.target === 'explorer') {
      const newWidth = Math.max(160, Math.min(480, state.startWidth + deltaX));
      setExplorerWidth(newWidth);
    } else {
      const newWidth = Math.max(280, Math.min(600, state.startWidth - deltaX));
  ...
  return (
    <div className="hcode-app" ref={containerRef}>
      ...
      <div className="hcode-titlebar">
        <span className="hcode-titlebar__logo">Hcode</span>
        <span className="hcode-titlebar__project">
          {state.workDir ? state.workDir.split(/[\\/]/).pop() : 'No project open'}
        </span>
        {state.workDir && (
          <span className="hcode-titlebar__path" title={state.workDir}>
            {state.workDir}
          </span>
        )}
      </div>
      ...
      <div className="hcode-main">
        ...
        {!explorerCollapsed && (
          <div
            className={`hcode-panel hcode-explorer ${focus === 'explorer' ? 'has-focus' : ''}`}
            style={{ width: `${explorerWidth}px` }}
            onClickCapture={() => setFocus('explorer')}
          >
            <div className="hcode-panel-header">
              ...
              <button
                ...
                onClick={(e) => { e.stopPropagation(); setExplorerCollapsed(true); }}
                title="Collapse (Ctrl+B)"
                style={{ padding: '2px var(--space-1)' }}
              >
                ■
              </button>
            </div>
        ...
        {!explorerCollapsed && (
          <div
            className="hcode-resize-handle"
            onPointerDown={(e) => onDragStart('explorer', e)}
            onDoubleClick={() => setExplorerWidth(220)}
          />
        )}

        {/* Collapsed Strip (Explorer) */}
        {explorerCollapsed && (
          <div
            className="hcode-panel-collapsed-strip"
            onClick={() => setExplorerCollapsed(false)}
            title="Expand Explorer (Ctrl+B)"
            style={{ width: '36px', borderRight: '1px solid var(--border-default)', background: 'var(--surface-1)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
        ...
        {/* Center: Editor */}
        <div
          className={`hcode-panel hcode-editor-panel ${focus === 'editor' ? 'has-focus' : ''}`}
          onClickCapture={() => setFocus('editor')}
        >
          {showDiffReview && state.patches.length > 0 ? (
            ...
            <>
              <div className="hcode-panel-header hcode-editor-tabs">
                <button className="hcode-editor-tab hcode-editor-tab--active">
            ...
            <WelcomeScreen
              ...
              onNewTask={() => { setAgentCollapsed(false); setFocus('agent'); }}
              onOpenSettings={() => setShowSettings(true)}
              recentFolders={recentFolders}
              onOpenRecent={() => handleSelectFolder()}
            />
          )}
        </div>
        ...
        {!agentCollapsed && (
          <div
            className="hcode-resize-handle"
            onPointerDown={(e) => onDragStart('agent', e)}
            onDoubleClick={() => setAgentWidth(360)}
          />
        )}

        {/* Collapsed Strip (Agent) */}
        {agentCollapsed && (
          <div
            className="hcode-panel-collapsed-strip"
            onClick={() => setAgentCollapsed(false)}
            title="Expand Agent (Ctrl+J)"
            style={{ width: '36px', borderLeft: '1px solid var(--border-default)', background: 'var(--surface-1)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
        ...
        {!agentCollapsed && (
          <div
            className={`hcode-panel hcode-agent-panel ${focus === 'agent' ? 'has-focus' : ''}`}
            style={{ width: `${agentWidth}px` }}
            onClickCapture={() => setFocus('agent')}
          >
            {showSettings ? (
              <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                <div className="hcode-panel-header hcode-agent-header">
                  ...
                  <div className="hcode-agent-actions">
                    <span className="hcode-agent-icon" onClick={() => setShowSettings(false)} title="Close Settings">✕</span>
                  </div>
                </div>
      ...
      <StatusBar
        ...
        phase={state.phase}
        workDir={state.workDir}
        pendingPatchCount={state.patches.length}
...
function updateTreeChildren(tree: FileEntry[], parentPath: string, children: FileEntry[]): FileEntry[] {
  return tree.map(entry => {
    ...
    if (entry.children) {
      return { ...entry, children: updateTreeChildren(entry.children, parentPath, children) };
    }
    return entry;
  });
}
