    ...
    async def execute(
        ...
        try:
            ...
            if context and context.get("explore_first", True):
                explore_agent = ExploreAgent(self.provider, self.tool_registry)
    ...
    async def execute_with_agents(
        ...
        for agent_type in agent_types:
            if agent_type == AgentType.EXPLORE:
                agents.append(ExploreAgent(provider, self.tool_registry))
            elif agent_type == AgentType.PLAN:
                agents.append(PlanAgent(provider, self.tool_registry))
            elif agent_type == AgentType.IMPLEMENT:
                agents.append(ImplementAgent(provider, self.tool_registry))
        ...
        if parallel:
            ...
            for agent in agents:
                ...
                if result.success:
                    if agent.agent_type == AgentType.EXPLORE:
                        context["exploration"] = result.output
                    elif agent.agent_type == AgentType.PLAN:
                        context["plan"] = result.output

        return results

    async def auto_execute(self, task: str) -> AgentResult:
        """
        ...
        return results[-1] if results else AgentResult(
