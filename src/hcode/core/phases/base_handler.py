...
"""
    ...
    def _load_code_instructions(self, context: AgentContext) -> str:
        """Load project architecture from .hcode/hcode.md (same as hcode_architecture)."""
        # Note: This is intentionally identical to _load_hcode_architecture() for backward compatibility.
        # Both methods load the same file to ensure consistent context injection.
        try:
            arch_content = self.artifact_manager.load_artifact("hcode.md", context)
            if not arch_content or not arch_content.strip():
                return ""
            logger.debug("Loaded project architecture from .hcode/hcode.md")
            return f"\n---\n\n## PROJECT CONTEXT\n\n{arch_content.strip()}\n\n---\n"
        except Exception as e:
            logger.debug(f"Loading project context failed: {e}")
            return ""
    ...
    def _load_hcode_architecture(self, context: AgentContext) -> str:
        """Load project architecture from .hcode/hcode.md (generated by /init)."""
        try:
            arch_content = self.artifact_manager.load_artifact("hcode.md", context)
            if not arch_content or not arch_content.strip():
                return ""
            ...
            preamble = (
                ...
                "> **What this is**: `hcode.md` is the authoritative map of this project, "
                "generated by `/init`. It documents key files, modules, entry points, "
                "conventions, and architectural patterns.\n>\n"
                "> **How to use it**:\n"
                "> - Use it as your primary source for locating files — prefer it over blind Glob searches.\n"
                "> - When it names a file or module, trust that path. Verify with Glob only if you suspect staleness.\n"
                "> - Match the coding patterns, naming conventions, and module structure it describes.\n"
                "> - Do NOT invent file paths or module structures that contradict what is documented here.\n\n"
            )
            return f"\n---\n\n{preamble}{arch_content.strip()}\n\n---\n"
        except Exception as e:
...
"""
    ...
    @staticmethod
    def _clean_thinking_for_display(text: str) -> str:
        """
        Remove tool call JSON and noise from thinking content before display.

        The AI often embeds raw JSON tool calls inside <thinking> blocks.
        These are already shown by the tool executor — showing them again
        in the thinking panel is redundant noise.

        Removes:
        - ```json ... ``` code blocks that contain "tool" key
        - Bare JSON objects (brace-balanced) that contain "tool" key
        - "Tool: ..." label lines that precede JSON blocks
        - Long separator lines (-------)
        - Collapses excessive blank lines
        """
        # 1. Remove ```json ... ``` / ``` ... ``` blocks containing tool calls
        text = re.sub(
            r'```(?:json)?\s*\{.*?"tool".*?\}\s*```',
            '',
            text,
        ...
        # 2. Remove bare JSON tool call objects using brace counting
        chars: list = []
        i, n = 0, len(text)
        while i < n:
            if text[i] == '{':
                ...
                while j < n:
                    if text[j] == '{':
                        ...
                        if depth == 0:
                            block = text[i:j + 1]
                            if '"tool"' in block:
                                i = j + 1   # skip the entire JSON block
                            else:
                                chars.append('{')
                                i += 1
                            break
        ...
        # 3. Remove "Tool: ..." label lines (e.g. "Tool: List the scripts/ directory")
        text = re.sub(r'(?m)^[ \t]*Tool:.*$\n?', '', text)

        # 4. Remove long dash separator lines
        text = re.sub(r'(?m)^[ \t]*-{10,}[ \t]*$\n?', '', text)
    ...
    def _display_thinking_blocks(self, response: str) -> None:
        ...
        Shows <thinking> content ONLY, with tool call JSON stripped out
        since those are already rendered by the tool executor.
        ...
        for thinking_content in thinking_matches:
            thinking_text = self._clean_thinking_for_display(thinking_content)
