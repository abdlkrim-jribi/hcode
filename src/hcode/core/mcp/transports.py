...
# Type alias for the (read_stream, write_stream) pair returned by transports
...
class StdioTransport:
    ...
    async def connect(self) -> _Streams:
        ...
        Raises:
            MCPTransportError: If the connection attempt fails.
        """
        from mcp import StdioServerParameters
        from mcp.client.stdio import stdio_client

        logger.debug(
            "[%s] Connecting via stdio (command=%s %s)",
            self._config.id,
...
class HttpTransport:
    ...
    async def connect(self) -> _Streams:
        ...
        logger.debug(
            "[%s] Connecting via HTTP/SSE (url=%s)",
            self._config.id,
            self._config.url,
        )
...
class TransportFactory:
    ...
    def create(config: MCPServerConfig) -> StdioTransport | HttpTransport:
        ...
        Raises:
            ValueError: If the transport type is not recognised.
        """
        if config.transport == "stdio":
            ...
            raise ValueError(
                f"Unsupported transport type '{config.transport}' for server "
                f"'{config.id}'. Expected 'stdio' or 'http'."
            )
