...
# ---------------------------------------------------------------------------
# MCPToolBridge — one MCP tool as a native hcode BaseTool
# ---------------------------------------------------------------------------
...
class MCPToolBridge(BaseTool):
    ...
    # Class-level marker so callers can identify MCP-originated tools
    ...
    def __init__(self, tool_info: MCPToolInfo, client_manager: MCPClientManager) -> None:
        ...
        # Override the default name (class name) set by BaseTool.__init__
    ...
    async def execute(self, **kwargs) -> ToolResult:
        ...
        metadata = {"mcp_server": server_id, "mcp_tool": tool_name}
        ...
        try:
            ...
            return ToolResult(success=False, output=None, error=str(exc), metadata=metadata)
...
# ---------------------------------------------------------------------------
# MCPToolRegistry — registers / unregisters all MCP bridges in ToolManager
# ---------------------------------------------------------------------------
...
class MCPToolRegistry:
    ...
    def __init__(self, client_manager: MCPClientManager, tool_manager) -> None:
        ...
        # server_id -> set of lowercased tool names stored in ToolRegistry.tools
    ...
    async def register_all_mcp_tools(self) -> None:
        ...
        """
        all_tools: List[MCPToolInfo] = self._client_manager.list_all_tools()

        # Group by server so we can log counts per server
            ...
            for tool_info in tools:
                try:
                    ...
                    logger.warning(
                        ...
                        tool_info.name, server_id, exc,
                    )

            # Merge with any previously registered names for this server
            self._registered.setdefault(server_id, set()).update(registered_names)
            logger.info(
                "Registered %d MCP tool(s) from server '%s'",
                len(registered_names), server_id,
            )
    ...
    async def unregister_server_tools(self, server_id: str) -> None:
        ...
        if tool_names:
            logger.info(
                "Unregistered %d MCP tool(s) from server '%s'",
                len(tool_names), server_id,
            )
