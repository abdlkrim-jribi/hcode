...
class TodoReadTool(BaseTool):
    """
    Retrieves the current task list for the session.

    Use this tool to check what todos are currently active, their status,
    ...
    def set_todos(self, todos: List[Dict[str, str]]):
        """Set the current todos (called by TodoWrite)"""
    ...
    async def execute(self) -> ToolResult:
        ...
        try:
            ...
            for i, todo in enumerate(self._current_todos, 1):
                ...
                output_lines.append(f"{i}. {emoji} **{status_display}**: {content}")

                if status == "in_progress":
                    output_lines.append(f"   _Currently: {active_form}_")

            output_lines.append("")

            # Add summary
            total = len(self._current_todos)
            completed = sum(1 for t in self._current_todos if t.get("status") == "completed")
